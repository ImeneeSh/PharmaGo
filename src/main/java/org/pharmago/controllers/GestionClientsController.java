...
public class GestionClientsController implements Initializable {
    ...
    private void configurerBoutonAjouter() {
        btnAjouterClient.setOnAction(event -> {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/org/pharmago/views/AjouterClient.fxml"));
    ...
    private void voirLivraisons(Client client) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/org/pharmago/views/GestionLivraisons.fxml"));
            Parent root = loader.load();
